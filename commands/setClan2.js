...
module.exports = {
    name: 'setclan2',
    aliases: ['setclan2', 'setclantag2'],
    description: 'Set Clan 2',
    parameters: ['#TAG'],
    disabled: false,
    execute: async (message, args, bot, db) => {
        ...
        if (!tag2) statistics.updateOne({}, { $inc: { linkedClans: 1 } }); //add new linked clan (if clan has not been linked in the past)
        ...
        message.channel.send({ embeds: [{ color: green, description: `✅ Clan 2 successfully linked to ${badgeEmoji} **${clan.name}**!` }] });
