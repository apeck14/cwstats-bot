module.exports = {
  getAvgFame: (clan, isColosseum, dayOfWeek) => {
    const { fame, participants, periodPoints } = clan
    ...
    const attacksCompletedToday = participants.reduce((a, b) => a + b.decksUsedToday, 0)
  ...
  getProjFame: (clan, isColosseum, dayOfWeek) => {
    ...
    const winRate = currentPossibleFame === 0 ? 0 : fame / currentPossibleFame
  ...
  getRacePlacements: (race, isColosseum) => {
    ...
    const clansWithPointsSorted = Object.values(newRace)
      .filter((cl) => cl.fame > 0)
    ...
    for (let i = 0; i < clansWithPointsSorted.length; i++) {
      ...
      while (clansWithPointsSorted[i + 1] && clansWithPointsSorted[i + 1].fame === clan.fame) {
        clansWithSameFame.push(clansWithPointsSorted[i + 1].tag)
        i++
      }

      for (const tag of clansWithSameFame) newRace[tag].placement = place
