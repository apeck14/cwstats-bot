...
module.exports = {
  ...
  run: async (i, db, client) => {
    ...
    if (abbreviations.length >= 15)
      return errorMsg(
        i,
        "**Up to 15 abbreviations can be set.** Remove an existing abbreviation and try again."
      )

    if (!abbreviation.match(/^[0-9a-zA-Z]+$/))
      return errorMsg(i, "**Abbreviation must be alphanumeric.**")

    if (abbreviation.length > 5)
      return errorMsg(i, "**Abbreviation cannot be larger than 5 characters.**")

    if (abbreviations.find((a) => a.abbr === abbreviation))
      return errorMsg(
        i,
        "**This abbreviation is already in use.** Remove it and try again."
      )

    if (abbreviations.find((a) => a.tag === formatTag(tag)))
      return errorMsg(
        i,
        "**This clan is already in use.** Remove it and try again."
      )
