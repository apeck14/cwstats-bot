...
module.exports = {
    ...
    run: async (i, db, client) => {
        ...
        if (!linkedAccount) {
            ...
            return i.reply({ embeds: [{ color: green, description: `✅ Account linked to **${player.name}**!` }] });
        }
        //already linked to that tag
        else if (linkedAccount.tag === tag) {
            return i.reply({ embeds: [{ color: orange, description: "**You have already linked that ID!**" }], ephemeral: true });
        }
