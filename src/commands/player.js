...
module.exports = {
    ...
    run: async (i, db, client) => {
        ...
        if (!user && !iTag) { //linked account
            ...
            else return i.reply({ embeds: [{ color: orange, description: `**No tag linked!**` }], ephemeral: true });
        }
            ...
            if (linkedAccount.tag) tag = linkedAccount.tag;
            else return i.reply({ embeds: [{ color: orange, description: `<@!${user.id}> **does not have an account linked.**` }], ephemeral: true });
        }
