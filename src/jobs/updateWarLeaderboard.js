...
module.exports = {
  ...
  run: async (client, db) => {
    ...
    const trackedIDs = locations
      .filter((l) => l.isAdded || l.name === "Global")
      .map((l) => l.id)

    const lbPromises = trackedIDs.map((id) =>
      getWarLeaderboard(id === "global" ? 100 : 75, id)
    )
    const allLbs = await Promise.all(lbPromises)

    const { data: allGlobalRankedClans, error: allGlobalRankedError } =
      await getWarLeaderboard(1000)

    if (allLbs.some((lb) => lb.error) || allGlobalRankedError)
      return console.log("Error while updating leaderboard!")

    const clansToCheckRaces = uniqBy(
      allLbs.map((lb) => lb.data).flat(),
      "tag"
    ...
    for (const c of clansToCheckRaces) {
      ...
      for (const cl of race.clans) {
        ...
        const globalClanRank = allGlobalRankedClans.findIndex(
          (cla) => cla.tag === cl.tag
        )
        ...
        if (cl.tag === c.tag) {
          clanAverages.push({
            ...
            decksRemaining:
              200 - cl.participants.reduce((a, b) => a + b.decksUsedToday, 0),
